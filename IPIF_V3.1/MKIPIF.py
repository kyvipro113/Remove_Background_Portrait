...
def check_config(data):
    try:
        ...
        if not check_dir_exists(DATA_IN):
            print("DATA_IN is not found")
            return False
        DATA_OUT = image_processing_config["DATA_OUT"]
        if not check_dir_exists(DATA_OUT):
        ...
        if not is_boolean(USE_OVAL_MASK):
            print("USE_OVAL_MASK must be bool (True or False)")
            return False
        PERCENT_PADDING_X = image_processing_config["PERCENT_PADDING_X"]
        if type(PERCENT_PADDING_X) != float:
            print("PERCENT_PADDING_X must be float. Recommend from 0.15 to 0.3")
            return False
        PERCENT_PADDING_Y = image_processing_config["PERCENT_PADDING_Y"]
        if type(PERCENT_PADDING_Y) != float:
            print("PERCENT_PADDING_Y must be float. Recommend from 0.25 to 0.35")
            return False
