...
from Processing_Image_Interface.utilities.Append_Oval_Mask import *
...
class Class_Config(object):
    ...
    stdev: list
    model_path_al: str
    al_use_gpu: bool = False
    num_threads_pytorch: int
    equal_hist: bool = False
    gray_scale: bool = False
    use_oval_mask: bool = False
    percent_padding_x: float
    percent_padding_y: float
...
def load_settings_image_processing(model_name_seg: str, model_path_seg: str, tensor_format: str, H: int, W: int, 
                                   ...
                                   use_oval_mask: bool, percent_padding_x: float, percent_padding_y,
    ...
    Class_Config.use_oval_mask = use_oval_mask
    Class_Config.percent_padding_x = percent_padding_x
    Class_Config.percent_padding_y = percent_padding_y
...
def process_remove_background_image(data_path: str, img_size_inp: str, lImage: list, data_out_path: str, name_logger_sub_process: str):
    ...
    use_oval_mask = Class_Config.use_oval_mask
    percent_padding_x = Class_Config.percent_padding_x
    percent_padding_y = Class_Config.percent_padding_y
    ...
    for img_file in lImage:
        ...
        if(img_size_inp == "Large"):
            ...
            if use_oval_mask:
                ...
                # img_pil_raw = Image.fromarray(im_rgb)
            h, w, c = image.shape
            if(image_flex):
                if Class_Config.H == "Flex":
                    H = h * W // w
        ...
        if use_oval_mask:
            #I_pil = append_oval_mask(detector=detector, raw_image=img_pil_raw, remove_bk_image=I_pil, percent_padding_x=percent_padding_x, percent_padding_y=percent_padding_y)
            I_pil = append_oval_mask(remove_bk_image=I_pil)
