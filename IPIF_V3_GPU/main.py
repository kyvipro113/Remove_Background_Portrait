...
def check_config(data):
    try:
        ...
        if MODEL_NAME_SEG == "portrait":
            ...
            if not is_boolean(USE_PADDING):
                print("USE_PADDING must be bool (True or False)")
                return False
            PADDING_VALUE = segmentation_model_config["PADDING_VALUE"]
            if type(PADDING_VALUE) != int:
                print("PADDING_VALUE must be int. Value range from 0 to 255. (Recommend value equal to 128)")
                return False
            SCALE = segmentation_model_config["SCALE"]
            if type(SCALE) != int:
                print("SCALE must be int. (Recommend value euqual to 1)")
                return False
            MEAN = segmentation_model_config["MEAN"]
            if type(MEAN) != list:
                print("MEAN must be list. (Recommend mean value equal to [103.94, 116.78, 123.68])")
                return False
            STDEV = segmentation_model_config["STDEV"]
            if type(STDEV) != list:
                print("STDEV must be list. (Recommend standard deviation equal to [0.017, 0.017, 0.017])")
                return False
